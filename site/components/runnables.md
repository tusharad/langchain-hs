---
title: Runnables & AST Composition
description: First-class AST composition via pipe (|>>), parallel (&>&), and fallback (>>>#) operators.
category: Components
---

<div class="component-header-meta">
  <span class="badge badge-primary">Langchain.Core.Runnable</span>
</div>
...
- **Type-Safe Operators**: `|>>` for sequential composition, `&>&` for parallel fan-out, and `>>>#` for fallback chains.
- **Uniform Invocation**: Every component—models, prompts, tools, chains, retrievers—implements the `Runnable` typeclass.
...
::: {.provider-group}
::: {.provider-panel data-provider="ollama" data-label="🦙 Ollama (Local)" data-exe="stack run runnableollama"}
```haskell
...
::: {.provider-panel data-provider="openai" data-label="⚡ OpenAI / OpenRouter" data-exe="stack run runnableopenai"}
```haskell
{-# LANGUAGE FlexibleContexts #-}
{-# LANGUAGE OverloadedStrings #-}
...
```haskell
Runnable r => r a b -> a -> ExceptT LangchainError IO b
```
```haskell
Runnable r => r a b -> r b c -> RunnableTree a c
```
```haskell
Runnable r => r a b -> r a c -> RunnableTree a (b, c)
```
```haskell
Runnable r => r a b -> r a b -> RunnableTree a b
```
