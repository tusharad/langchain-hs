...
let handler = OpenAIStreamHandler
      { onToken = \chunk -> putStr (maybe "" id $ contentForDelta $ chunkChoiceDelta $ head $ chunkChoices chunk)
...
Contains a list of possible choices for the completion and metadata.
-}
data ChatCompletionChunk = ChatCompletionChunk
  ...
  , chunkCreated :: Integer
  -- ^ Unix timestamp of when the chunk was created
  , chunkModel :: Text
  -- ^ The model used for the completion
  , chunkObject :: Text
  -- ^ Object type, always "chat.completion.chunk"
  , chunkServiceTier :: Maybe Text
  -- ^ Service tier used
  , chunkSystemFingerprint :: Maybe Text
  -- ^ System fingerprint
  , chunkUsage :: Maybe Usage
  -- ^ Optional usage information (only present when stream_options.include_usage is true)
  }
  deriving (Show)
...
Includes the incremental content, index, and an optional reason for finishing.
-}
data ChunkChoice = ChunkChoice
  { chunkChoiceDelta :: Delta
  -- ^ Incremental content added in this chunk
  , chunkChoiceIndex :: Int
  -- ^ Index of the choice in the list of choices
  , chunkChoiceFinishReason :: Maybe FinishReason
  -- ^ Reason why the completion stopped, if applicable
  , chunkChoiceLogprobs :: Maybe LogProbs
...
data Delta = Delta
  ...
  -- ^ Optional role of the author of this message
  , deltaToolCalls :: Maybe [ToolCall]
  -- ^ Optional tool calls generated in this chunk
  , deltaRefusal :: Maybe Text
  -- ^ Optional refusal message generated by the model
  , deltaFunctionCall :: Maybe FunctionCallInfo
  -- ^ Optional function call (deprecated, replaced by tool_calls)
  }
  deriving (Show)
...
data ChatCompletionRequest = ChatCompletionRequest
  ...
  -- ^ Bias for specific tokens (values from -100 to 100)
  , logprobs :: Maybe Bool
  ...
  , user :: Maybe Text
  -- ^ User identifier (deprecated, use safetyIdentifier or promptCacheKey)
  ...
  -- ^ Verbosity level of the model's response
  , safetyIdentifier :: Maybe Text
  -- ^ Stable identifier for detecting policy violations
  , promptCacheKey :: Maybe Text
  -- ^ Key for optimizing cache hit rates
  , requestFunctionCall :: Maybe (Either Text FunctionCallInfo)
  -- ^ Deprecated: Controls function calling (replaced by toolChoice)
  , requestFunctions :: Maybe [FunctionDef]
  -- ^ Deprecated: List of functions (replaced by tools)
  }
  ...
  toJSON ChatCompletionRequest {..} =
    object $
      ...
      , "model" .= model
        ...
        ++ maybe [] (\v -> ["verbosity" .= v]) verbosity
        ++ maybe [] (\si -> ["safety_identifier" .= si]) safetyIdentifier
        ++ maybe [] (\pck -> ["prompt_cache_key" .= pck]) promptCacheKey
        ++ maybe [] (\fc -> ["function_call" .= fc]) requestFunctionCall
        ++ maybe [] (\fs -> ["functions" .= fs]) requestFunctions
...
data Message = Message
  ...
  -- ^ Optional annotations (e.g., URL citations from web search)
  , messageFunctionCall :: Maybe FunctionCallInfo
  -- ^ Optional function call (deprecated, replaced by toolCalls)
  }
  deriving (Show, Eq, Generic)
  ...
  toJSON Message {..} =
    object $
      ["role" .= role]
        ...
        ++ maybe [] (\ann -> ["annotations" .= ann]) annotations
        ++ maybe [] (\fc -> ["function_call" .= fc]) messageFunctionCall
...
{- | Represents a deprecated function call structure.
Used for backward compatibility with deprecated function_call field.
-}
data FunctionCallInfo = FunctionCallInfo
  { functionCallName :: Text
  -- ^ Name of the function to call
  , functionCallArguments :: Text
  -- ^ Arguments for the function in JSON format
  }
  deriving (Show, Eq, Generic)

instance ToJSON FunctionCallInfo where
...
instance FromJSON FunctionCallInfo where
  parseJSON = withObject "FunctionCallInfo" $ \v ->
    FunctionCallInfo
      <$> v .: "name"
      <*> v .: "arguments"
...
data Verbosity = VerbosityLow | VerbosityMedium | VerbosityHigh
  deriving (Show, Eq, Generic)

instance ToJSON Verbosity where
  toJSON VerbosityLow = String "low"
  toJSON VerbosityMedium = String "medium"
  toJSON VerbosityHigh = String "high"

instance FromJSON Verbosity where
  parseJSON (String "low") = return VerbosityLow
  parseJSON (String "medium") = return VerbosityMedium
  parseJSON (String "high") = return VerbosityHigh
  parseJSON invalid = fail $ "Invalid verbosity: " ++ show invalid

-- | Represents a URL citation annotation.
data UrlCitation = UrlCitation
  { urlCitationStartIndex :: Int
  -- ^ The index of the first character of the URL citation
  , urlCitationEndIndex :: Int
  -- ^ The index of the last character of the URL citation
  , urlCitationUrl :: Text
  -- ^ The URL of the web resource
  , urlCitationTitle :: Text
  -- ^ The title of the web resource
  }
  deriving (Show, Eq, Generic)

instance ToJSON UrlCitation where
  toJSON UrlCitation {..} =
    object
      [ "start_index" .= urlCitationStartIndex
      , "end_index" .= urlCitationEndIndex
      , "url" .= urlCitationUrl
      , "title" .= urlCitationTitle
      ]

instance FromJSON UrlCitation where
  parseJSON = withObject "UrlCitation" $ \v ->
    UrlCitation
      <$> v .: "start_index"
      <*> v .: "end_index"
      <*> v .: "url"
...
-- | Represents an annotation for a message.
data Annotation = Annotation
  { annotationType :: Text
  -- ^ Type of annotation, always "url_citation"
...
instance FromJSON Annotation where
  parseJSON = withObject "Annotation" $ \v ->
    Annotation
      <$> v .: "type"
      <*> v .: "url_citation"
...
data StreamOptions = StreamOptions
  ...
  -- ^ Whether to enable stream obfuscation for security
  }
  deriving (Show, Eq, Generic)
...
{- | Represents approximate location for web search.
Contains optional geographic and timezone information.
-}
data ApproximateLocation = ApproximateLocation
  { locationCountry :: Maybe Text
  -- ^ Two-letter ISO country code (e.g., "US")
  , locationRegion :: Maybe Text
  -- ^ Free text for region (e.g., "California")
  , locationCity :: Maybe Text
  -- ^ Free text for city (e.g., "San Francisco")
...
instance ToJSON ApproximateLocation where
  toJSON ApproximateLocation {..} =
    object $
      maybe [] (\c -> ["country" .= c]) locationCountry
        ++ maybe [] (\r -> ["region" .= r]) locationRegion
        ++ maybe [] (\c -> ["city" .= c]) locationCity
        ++ maybe [] (\t -> ["timezone" .= t]) locationTimezone
...
defaultChatCompletionRequest =
  ChatCompletionRequest
    ...
    , requestFunctionCall = Nothing
    , requestFunctions = Nothing
    }
...
defaultUserLocation =
  UserLocation
    { userLocationType = "approximate"
    , approximate = defaultApproximateLocation
    }

-- | Default approximate location with no specific details.
defaultApproximateLocation :: ApproximateLocation
defaultApproximateLocation =
  ApproximateLocation
    { locationCountry = Nothing
    , locationRegion = Nothing
    , locationCity = Nothing
    , locationTimezone = Nothing
    }

-- | Default verbosity (Medium).
defaultVerbosity :: Verbosity
defaultVerbosity = VerbosityMedium
