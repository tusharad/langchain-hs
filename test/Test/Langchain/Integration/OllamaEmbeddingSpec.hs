...
import Langchain.VectorStore.InMemory (InMemory, emptyInMemoryVectorStore, fromDocuments)
...
tests =
  testGroup
    ...
    [ testCase "Ollama live embeddings and vector similarity search" $ do
        withOllamaModel defaultEmbedModel $ \modelName -> do
          let embedModel = OllamaEmbeddings modelName Nothing Nothing Nothing
              docs =
          ...
          case resStore of
            ...
            Right (store :: InMemory OllamaEmbeddings) -> do
              ...
              case resSearch of
                ...
                Right matches -> do
                  length matches @?= 1
                  assertBool "Top match is Haskell" ("Haskell" `TL.isInfixOf` pageContent (head matches))
    ]
