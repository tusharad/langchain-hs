...
tests :: TestTree
tests = testGroup "Langchain.VectorStore.Filter"
  ...
  , testCase "And & Or composite filters evaluate correctly" $ do
      let doc1 = Document { pageContent = "Doc 1", metadata = Map.fromList [("env", String "prod"), ("tier", Number 1)] }
          doc2 = Document { pageContent = "Doc 2", metadata = Map.fromList [("env", String "staging"), ("tier", Number 1)] }
          doc3 = Document { pageContent = "Doc 3", metadata = Map.fromList [("env", String "prod"), ("tier", Number 2)] }
