...
tests :: TestTree
tests = testGroup "Langchain.Retriever.BM25"
  [ testCase "BM25 finds exact matching document" $ do
      let doc1 = Document { pageContent = "Haskell is a functional programming language with strong static types.", metadata = Map.empty }
          doc2 = Document { pageContent = "Python is a dynamic language used for machine learning and web scripts.", metadata = Map.empty }
          doc3 = Document { pageContent = "Rust guarantees memory safety without garbage collection.", metadata = Map.empty }
  ...
  , testCase "BM25 scoring gives highest score to relevant passage" $ do
      let doc1 = Document { pageContent = "Deep research agent explores web pages and validates claims.", metadata = Map.empty }
          doc2 = Document { pageContent = "Database query optimization and index scans in postgresql.", metadata = Map.empty }
